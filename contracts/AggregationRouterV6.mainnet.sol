 ...
 * @notice A library to manage and check MakerTraits, which are used to encode the maker's preferences for an order in a
 * single uint256.
 * @dev
...
library MakerTraitsLib {
     ...
     * @dev If the `HAS_EXTENSION_FLAG` is set in the makerTraits, then the protocol expects that the order has
     * extension(s).
 ...
 * @notice This library to manage and check TakerTraits, which are used to encode the taker's preferences for an order
 * in a single uint256.
 * @dev The TakerTraits are structured as follows:
 * High bits are used for flags
 * 255 bit `_MAKER_AMOUNT_FLAG`           - If set, the taking amount is calculated based on making amount, otherwise
 * making amount is calculated based on taking amount.
 * 254 bit `_UNWRAP_WETH_FLAG`            - If set, the WETH will be unwrapped into ETH before sending to taker.
 * 253 bit `_SKIP_ORDER_PERMIT_FLAG`      - If set, the order skips maker's permit execution.
 * 252 bit `_USE_PERMIT2_FLAG`            - If set, the order uses the permit2 function for authorization.
 * 251 bit `_ARGS_HAS_TARGET`             - If set, then first 20 bytes of args are treated as target address for
 * maker’s funds transfer.
 * 224-247 bits `ARGS_EXTENSION_LENGTH`   - The length of the extension calldata in the args.
 * 200-223 bits `ARGS_INTERACTION_LENGTH` - The length of the interaction calldata in the args.
 * 0-184 bits                             - The threshold amount (the maximum amount a taker agrees to give in exchange
 * for a making amount).
 */
 ...
 * @notice The order filling steps are `preInteraction` =>` Transfer "maker -> taker"` => **`Interaction`** => `Transfer
 * "taker -> maker"` => `postInteraction`
 */
interface ITakerInteraction {
    /**
     * @dev This callback allows to interactively handle maker aseets to produce takers assets, doesn't supports ETH as
     * taker assets
...
library OffsetsLib {
    ...
    /**
     * @notice Retrieves the field value calldata corresponding to the provided field index from the concatenated
     * calldata.
     * @dev
     ...
     * 3. Throw `OffsetOutOfBounds` error if the length of the segment is greater than the length of the concatenated
     * data.
     ...
     * @param index The index of the segment to retrieve. The field index 0 corresponds to the lowest bytes of the
     * offsets array.
...
library ECDSA {
    ...
    {
        // (bool success, bytes memory data) =
        // signer.staticcall(abi.encodeWithSelector(IERC1271.isValidSignature.selector, hash, abi.encodePacked(r, vs)));
        // return success && data.length >= 4 && abi.decode(data, (bytes4)) == IERC1271.isValidSignature.selector;
        bytes4 selector = IERC1271.isValidSignature.selector;
        assembly ("memory-safe") {
            // solhint-disable-line no-inline-assembly
    ...
    function toEthSignedMessageHash(bytes32 hash) internal pure returns (bytes32 res) {
        ...
        assembly ("memory-safe") {
            // solhint-disable-line no-inline-assembly
            mstore(0, 0x19457468657265756d205369676e6564204d6573736167653a0a333200000000) // "\x19Ethereum Signed
                // Message:\n32"
            mstore(28, hash)
            res := keccak256(0, 60)
        }
...
library OrderLib {
     ...
     * @return receiver The address of the receiver, either explicitly defined in the order or the maker's address if
     * not specified.
     */
 ...
 * Each bit of the integer represents whether the object with corresponding index is valid or has been invalidated (0 -
 * valid, 1 - invalidated).
...
library SafeERC20 {
     ...
     * @param permit2 If true, uses the Permit2 standard for the transfer; otherwise uses the standard ERC20
     * transferFrom.
     */
     ...
     * @dev Increases with safe math check. Checks if the increased allowance will overflow, if yes, then it reverts the
     * transaction.
     ...
     * @dev Decreases with safe math check. Checks if the decreased allowance will underflow, if yes, then it reverts
     * the transaction.
     * Then uses `forceApprove` to increase the allowance.
     * Note that the implementation does not perform dirty bits cleaning, so it is the responsibility of
     * the caller to make sure that the higher 96 bits of the `spender` parameter are clean.
    ...
    /**
     * @notice Attempts to execute the `permit` function on the provided token with the sender and contract as
     * parameters.
     ...
     * @dev The function is designed to support a variety of permit functions, namely: IERC20Permit, IDaiLikePermit, and
     * IPermit2.
        ...
        assembly ("memory-safe") {
            ...
            case 100 {
                ...
                // Compact IERC20Permit.permit(uint256 value, uint32 deadline, uint256 r, uint256 vs)
                {
                    ...
                    mstore(add(ptr, 0x84), add(27, shr(255, vs))) // store v         = most significant bit of vs + 27
                        // (27 or 28)
                    calldatacopy(add(ptr, 0xa4), add(permit.offset, 0x24), 0x20) // store r         = copy permit.offset
                        // 0x24..0x43
                    mstore(add(ptr, 0xc4), shr(1, shl(1, vs))) // store s         = vs without most significant bit
                }
                // IERC20Permit.permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32
                // r, bytes32 s)
            ...
            case 72 {
                ...
                // Compact IDaiLikePermit.permit(uint32 nonce, uint32 expiry, uint256 r, uint256 vs)
                {
                    ...
                    mstore(add(ptr, 0x44), shr(224, calldataload(permit.offset))) // store nonce   = copy permit.offset
                        // 0x00..0x03
                    mstore(add(ptr, 0x64), sub(expiry, 1)) // store expiry  = expiry - 1
                    mstore(add(ptr, 0x84), true) // store allowed = true
                    mstore(add(ptr, 0xa4), add(27, shr(255, vs))) // store v       = most significant bit of vs + 27 (27
                        // or 28)
                    calldatacopy(add(ptr, 0xc4), add(permit.offset, 0x08), 0x20) // store r       = copy permit.offset
                        // 0x08..0x27
                    mstore(add(ptr, 0xe4), shr(1, shl(1, vs))) // store s       = vs without most significant bit
                }
                // IDaiLikePermit.permit(address holder, address spender, uint256 nonce, uint256 expiry, bool allowed,
            ...
            // Compact IPermit2
            case 96 {
                // Compact IPermit2.permit(uint160 amount, uint32 expiration, uint32 nonce, uint32 sigDeadline, uint256
                // r, uint256 vs)
                ...
                mstore(add(ptr, 0x64), and(0xffffffffffff, sub(shr(224, calldataload(add(permit.offset, 0x14))), 1))) // store
                    // expiration = ((permit.offset 0x14..0x17 - 1) & 0xffffffffffff)
                mstore(add(ptr, 0x84), shr(224, calldataload(add(permit.offset, 0x18)))) // store nonce = copy
                    // permit.offset 0x18..0x1b
                mstore(add(ptr, 0xa4), spender) // store spender
                // and(0xffffffffffff, ...) - conversion to uint48
                mstore(add(ptr, 0xc4), and(0xffffffffffff, sub(shr(224, calldataload(add(permit.offset, 0x1c))), 1))) // store
                    // sigDeadline = ((permit.offset 0x1c..0x1f - 1) & 0xffffffffffff)
                mstore(add(ptr, 0xe4), 0x100) // store offset = 256
                mstore(add(ptr, 0x104), 0x40) // store length = 64
                calldatacopy(add(ptr, 0x124), add(permit.offset, 0x20), 0x20) // store r      = copy permit.offset
                    // 0x20..0x3f
                calldatacopy(add(ptr, 0x144), add(permit.offset, 0x40), 0x20) // store vs     = copy permit.offset
                    // 0x40..0x5f
    ...
    /**
     * @dev Executes a low level call to a token contract, making it resistant to reversion and erroneous boolean
     * returns.
     * @param token The IERC20 token contract on which the call will be made.
     * @param selector The function signature that is to be called on the token contract.
     * @param to The address to which the token amount will be transferred.
     * @param amount The token amount to be transferred.
     * @return success A boolean indicating if the call was successful. Returns 'true' on success and 'false' on
     * failure.
    ...
    /**
     * @notice Safely deposits a specified amount of Ether into the IWETH contract. Consumes less gas then regular
     * `IWETH.deposit`.
    ...
    /**
     * @notice Safely withdraws a specified amount of wrapped Ether from the IWETH contract. Consumes less gas then
     * regular `IWETH.withdraw`.
            ...
            if (!takerTraits.skipMakerPermit()) {
                ...
                if (makerPermit.length >= 20) {
                    ...
                    if (!order.makerTraits.useBitInvalidator()) {
                        // Bit orders are not subjects for reentrancy, but we still need to check remaining-based orders
                        // for reentrancy
...
library StringUtil {
    ...
    function toHex(bytes memory data) internal pure returns (string memory result) {
        assembly ("memory-safe") {
            ...
            } lt(fromPtr, endPtr) { fromPtr := add(fromPtr, 0x20) } {
...
library UniERC20 {
    ...
    function _uniDecode(IERC20 token, bytes4 lowerCaseSelector, bytes4 upperCaseSelector)
        private
        view
        returns (string memory result)
    {
        ...
        (bool success, bytes memory data) =
            address(token).staticcall{gas: 20000}(abi.encodeWithSelector(lowerCaseSelector));
        if (!success) {
...
contract ClipperRouter is Pausable, EthReceiver {
    ...
    function clipperSwapTo(
        ...
        if (srcToken_ == _ETH) {
            // clipperExchange.sellEthForToken{value: inputAmount}(address(dstToken), inputAmount, outputAmount,
            // goodUntil, recipient, signature, _INCH_TAG);
            ...
            // clipperExchange.sellTokenForEth(address(srcToken_), inputAmount, outputAmount, goodUntil, recipient,
            // signature, _INCH_TAG);
            address clipper = address(clipperExchange);
            ...
            // clipperExchange.swap(address(srcToken_), address(dstToken), inputAmount, outputAmount, goodUntil,
            // recipient, signature, _INCH_TAG);
            address clipper = address(clipperExchange);
...
/**
 * @title UnoswapRouter
 * @notice A router contract for executing token swaps on Unoswap-compatible decentralized exchanges: UniswapV3,
 * UniswapV2, Curve.
 */
contract UnoswapRouter is Pausable, EthReceiver, IUniswapV3SwapCallback {
    ...
    /**
     * @notice Swaps ETH for another token using an Unoswap-compatible exchange's pool, with a minimum return specified
     * by `minReturn`.
     *         The function is payable and requires the sender to attach ETH.
     *         It is necessary to check if it's cheaper to use _WETH_NOT_WRAP_FLAG in `dex` Address (for example: for
     * Curve pools).
     * @param minReturn The minimum amount of tokens to be received after the swap.
     * @param dex The address of the Unoswap-compatible exchange's pool.
     * @return returnAmount The actual amount of tokens received after the swap.
    ...
    /**
     * @notice Swaps ETH for another token using an Unoswap-compatible exchange's pool, sending the resulting tokens to
     * the `to` address,
     *         with a minimum return specified by `minReturn`. The function is payable and requires the sender to attach
     * ETH.
     *         It is necessary to check if it's cheaper to use _WETH_NOT_WRAP_FLAG in `dex` Address (for example: for
     * Curve pools).
     * @param to The address to receive the swapped tokens.
    ...
    /**
     * @notice Swaps `amount` of the specified `token` for another token using two Unoswap-compatible exchange pools
     * (`dex` and `dex2`) sequentially,
     *         with a minimum return specified by `minReturn`.
     * @param token The address of the token to be swapped.
     * @param amount The amount of tokens to be swapped.
     * @param minReturn The minimum amount of tokens to be received after the swap.
    ...
    function _unoswapTo2(
        ...
        Address dex,
        Address dex2
    ) private whenNotPaused returns (uint256 returnAmount) {
    ...
    /**
     * @notice Swaps `amount` of the specified `token` for another token using three Unoswap-compatible exchange pools
     *         (`dex`, `dex2`, and `dex3`) sequentially, sending the resulting tokens to the `to` address, with a
     * minimum return specified by `minReturn`.
     * @param to The address to receive the swapped tokens.
     * @param token The address of the token to be swapped.
     * @param amount The amount of tokens to be swapped.
     * @param minReturn The minimum amount of tokens to be received after the swap.
    ...
    /**
     * @notice Swaps ETH for another token using three Unoswap-compatible exchange pools (`dex`, `dex2`, and `dex3`)
     * sequentially,
     *         with a minimum return specified by `minReturn`. The function is payable and requires the sender to attach
     * ETH.
     *         It is necessary to check if it's cheaper to use _WETH_NOT_WRAP_FLAG in `dex` Address (for example: for
        ...
        returns (uint256 ret)
    {
        ...
        assembly ("memory-safe") {
            ...
            // this will not overflow as reserve0, reserve1 and ret fit to 112 bit and numerator and _DENOMINATOR fit to
            // 32 bit
    ...
    // Curve Pool function selectors for different `exchange` methods. For details, see
    ...
    function _curfe(address recipient, uint256 amount, uint256 minReturn, Address dex) private returns (uint256 ret) {
        ...
        assembly ("memory-safe") {
            ...
            function asmApprove(token, to, value, mem) {
                ...
                let exception := 0x3e3f8f7300000000000000000000000000000000000000000000000000000000 // error
                    // ApproveFailed()
            ...
            // use approve when the callback is not used AND (raw ether is not used at all OR ether is used on the
            // output)
                ...
                if xor(recipient, address()) {
                    ...
                    default {
                        ...
                        if iszero(
                            ...
                            mstore(ptr, 0xf27f64e400000000000000000000000000000000000000000000000000000000) // error
                                // ERC20TransferFailed()
                            revert(ptr, 4)
                        }
                    }
...
/// @notice Main contract incorporates a number of routers to perform swaps and limit orders protocol to fill limit
/// orders
contract AggregationRouterV6 is
    EIP712("1inch Aggregation Router", "6"),
    Ownable,
    Pausable,
    ClipperRouter,
    GenericRouter,
    ...
    OrderMixin
{
     ...
     * construction.
     */
    constructor(IWETH weth) ClipperRouter(weth) OrderMixin(weth) Ownable(msg.sender) {
